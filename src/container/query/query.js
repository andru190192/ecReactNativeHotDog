...
const Profile = ({navigation}) => {
    ...
    const onHandlerPicker = async () => {
        const image = await ImagePicker.openPicker({width: 300, height: 400, cropping: true});
    ...
    const validateIsHotDog = async (image) => {
        ...
        const isHotDogLabel = responseJson.responses[0].labelAnnotations.map(label => label.description).includes('Hot dog');
